...
public class Util {
	...
	public static LinkedList<Object[]> getSignupData(String sheetName1,Xls_Reader xls) {
		
		LinkedList<Object[]> signupdata = new LinkedList<Object[]>();
		...
		try {
			...
			for (int i = dataStartRowNum; i <= (dataStartRowNum + totalRows); i++) {
				...
				if (xls.getCellData(sheetName1, 0, i).equalsIgnoreCase("Signup")) {
	                 dataInitialization.setUfname(xls.getCellData(sheetName1, 1, i));
	                 dataInitialization.setUlname(xls.getCellData(sheetName1, 2, i));
